...
function CardTotalFolha({ colaboradorId }: Props) {
  ...
  useEffect(() => {
    async function carregarFolha() {
      try {
        setLoading(true);
        const response = await api.get(`/folha-pagamento/colaborador/${colaboradorId}`);
        const folhas = response.data;

        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const soma = folhas.reduce((acc: number, item: any) => acc + (item.salarioFinal ?? 0), 0);
        setTotalFolha(soma);
      } catch (error) {
  ...
  return (
    <div className="w-full flex justify-center mt-8 px-4">
      {/* overflow-hidden é vital aqui para o rodapé respeitar o rounded-xl */}
      <div className="bg-white rounded-xl shadow-md w-70 overflow-hidden flex flex-col">
        ...
        {/* Rodapé "Ver todos" colado nas bordas */}
        <button 
          ...
          onClick={() => window.alert("Ver detalhes da folha")}
        >
